...
/* initialize */
	...
	if(prec <= 1) {
		outcoord->mm = skycalc_round((outcoord->mm + outcoord->ss / 60.),prec);
		outcoord->ss = 0.;
		...
		outcoord->ss = skycalc_round(outcoord->ss,(prec-2));
...
/* Sorting package ...
 ...
 (b) it does work, and (c) it's legal.

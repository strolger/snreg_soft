/* rwfits.c: routines to read and write FITS files
 * 
 * rwfits 3.0 - 11/02/99 JT Major changes, add rffile.
 *
...
/*
 * rffile() reads FITS image from disk into existing header and data array
 */
rffile(nhead, head, nx,ny, bp_data, data, file)
...
char *data;		/* image data */
{
  ...
  int mefits, fitswab;
  if((err=rfits_(file, nhead, head, &bitpix, nx,ny, &scale,&zero, data,
		 strlen(file),80))     != 0) return(err);

  npix = (*nx) * (*ny);
  fitswab = 1;
  fitsconvert_(&fitswab, &npix, &scale, &zero, &bitpix, data, bp_data, data);

  return(0);
}

/*
 * wffile() writes a FITS image onto disk.
 */
wffile(nhead, head, bp_data, data, file)
int *nhead;		/* header count */
char *head;		/* header */
char *file;		/* file name */
...
char *data;		/* image data */
{
  int fd, mode=1, err, bitpix, nx, ny;
  int nwrite, nbyte, imagebyte, nwrit, npix, fitswab;
  double scale, zero;
...
/* Convert from machine specific type to desired output FITS format */
    ...
    fitsconvert_(&fitswab, &npix, &scale, &zero, bp_data, 
		 &data[(ABS(*bp_data)*nwrit)/ABS(bitpix)], &bitpix, buf);
...
fitsconvert_(fitswab, npix, scale, zero, bp_src, src, bp_dest, dest)
...
/* Check for disallowed BITPIX */
  ...
  /*
  printf("fitswab=%d npix=%d scale=%g zero=%g bp_src=%d bp_dest=%d\n",
	 *fitswab, *npix, *scale, *zero, *bp_src, *bp_dest);
  for(j=0; j<10; j++) printf("%6d",*((unsigned short*)src+j));
  printf("\n");
  */
...
/* Separate out the case involving IEEE -> fp because of VAX possibility */
    if(*bp_src == -32) ieeefp_(npix, src, src);
#ifdef LOWENDIAN    
  ...
  case -32:
    if(*bp_dest == -32) {
      if(src != dest) bcopy(src, dest, 4*(*npix));
    } else if(*bp_dest == 32) {
      FIconvert_(npix, scale, zero, src, dest);
    } else if(*bp_dest == -16) {
      FUconvert_(npix, scale, zero, src, dest);
    } else if(*bp_dest == 16) {
      FSconvert_(npix, scale, zero, src, dest);
    } else if(*bp_dest == 1) {
      FBconvert_(npix, scale, zero, src, dest);
    }
    break;

  case 32:
    if(*bp_dest == -32) {
      IFconvert_(npix, scale, zero, src, dest);
    } else if(*bp_dest == 32) {
      if(src != dest) bcopy(src, dest, 4*(*npix));
    }
    break;
  ...
  case -16:
    if(*bp_dest == -32) {
      UFconvert_(npix, scale, zero, src, dest);
    } else if(*bp_dest == -16) {
      UUconvert_(npix, scale, zero, src, dest);
    } else if(*bp_dest == 16) {
      USconvert_(npix, scale, zero, src, dest);
    } else if(*bp_dest == 1) {
      SBconvert_(npix, scale, zero, src, dest);
    }
    break;

  case 16:
    if(*bp_dest == -32) {
      SFconvert_(npix, scale, zero, src, dest);
    } else if(*bp_dest == -16) {
      SUconvert_(npix, scale, zero, src, dest);
    } else if(*bp_dest == 16) {
      SSconvert_(npix, scale, zero, src, dest);
    } else if(*bp_dest == 1) {
      SBconvert_(npix, scale, zero, src, dest);
    }
    break;
  ...
  case 1:
    if(*bp_dest == -32) {
      BFconvert_(npix, scale, zero, src, dest);
    } else if(*bp_dest == -16) {
      BSconvert_(npix, scale, zero, src, dest);
    } else if(*bp_dest == 16) {
      BSconvert_(npix, scale, zero, src, dest);
    } else if(*bp_dest == 1) {
...
/* Create space for the FITS header and data */
  nhead = nhead + 36;		/* Room for added entries */
  allocfits(nhead, head, naxis, dims, -32, data);
  bzero(*head, 80*nhead);
...
/*
 * wfitsshort() will write a 16 bit FITS floating point image onto disk.
 */
...
char **data;		/* Pointer to data array */
{
  ...
  *head = (char *)malloc(80*nhead + 1);
  ...
  nalloc = 1;
  while(--naxis >= 0) nalloc *= dims[naxis];
...
/* Sanity check against nulls in the header instead of spaces */
  for(j=0; j<i; j++) {
    if(head[j] == '\0') head[j] = ' ';
  }
...
double *scale, *zero;
{
  ...
  if(*scale == 1.0) {
    if(*zero == 0.0) {
      ...
      for(i=0; i<*npix; i++) {
	itemp = izero + *src++;
	*dest++ = TRUNCATE(-32767, 32768, itemp);
      }
    }
  } else {
    for(i=0; i<*npix; i++) {
      temp = (*scale) * ((*zero) + *src++);
      temp = TRUNCATE(-32768, 32768, temp);
      *dest++ = NINT(temp);
    }
  }
}
...
double *scale, *zero;
{
  ...
  *dp = 0;
  for(i=0; i<*npix; i++) {
    *dp = (*dp >> 1) | ((*src++ == 0.0) ? 0x00 : 0x80);
    if( (i%8) == 7 ) dp++;
  }
}


/* Older vintage routines: */
...
char *fp;		/* Really FP, but may need to access bytes */
{
  ...
  FITSorder(32,*npix,data);
#endif
  if(fp != data) bcopy(data,fp,4*(*npix));
#endif
}
...
#else
  if(fp != data) bcopy(fp,data,4*(*npix));
#ifdef LOWENDIAN
  FITSorder(32,*npix,data);
#endif
#endif

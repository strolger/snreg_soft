...
int main(int argc,char *argv[])
{
  ...
  char        s[256],s2[256],config_file[256],*map,check,kernel_name[256],
              table_name[256],*spt, line[256];
  FILE        *file,*file0,*file1,*file2,*kernel_table,*coeff_file,*sum_file,*stampfile;
  float       Xs[1000],Ys[1000]; /*added for stamp positions*/
...
/* Added by lgs 28jun12 to specify stamp locations*/
/* built from jt and bps additions to original isis routines*/
 
 if (stampsbyxy==1) {
   stampfile=fopen("STAMPS","r");
   for (i=0; feof(stampfile) == 0; i++) {
     fgets(line,150, stampfile); 
     if (sscanf(line,"%f %f",&Xs[i],&Ys[i]) <1) i--;
   }
   close (stampfile);
/* 000721 (JT) Brian's feof() construction always dups last vector */
   i--;
/* Also, must have error check lest we overflow stamp memory! */
   if(i > nstamps) {
     fprintf(stderr, 
	     "alardsub: Need more stamps! nstamp_x: %d, nstamp_y: %d; ext file: %d\n",
	     nstamps_x, nstamps_y, i);
     exit(1);
   }
   Xs[i]=0;
   Ys[i]=0;
   get_stamps_brian(sub_ref,sub_image,map,Xs,Ys); 
 }
 else {

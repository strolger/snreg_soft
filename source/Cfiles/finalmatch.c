     ...
     char *argv[];
{
  ...
  if ( argc != 14 ) {
     printf( "Usage: %s templist objlist matchstars ax bx cx ay by cy closerthan totalmatchlimit skipcol wcsout\n", argv[0] );
     exit( 1 );
  }
  ...
  sscanf(argv[8],"%f",&cy);
  sscanf(argv[9],"%f",&ay);
  sscanf(argv[10],"%f",&limit);
  sscanf(argv[11],"%d",&nobj);
  sscanf(argv[12],"%d",&skiprow);
  sscanf(argv[13],"%d",&wcsout);
  ...
  for (i=1; feof(stream1) == 0 && i < nobj && i < MOBJ; i++) {
    ...
    if (skiprow ==0) {
      if(strncmp(line, "#", 1) != 0) {
	if (sscanf(line,"%f %f %s %s %s %s",&x1[i],&y1[i],dummyc,dummy1[i],dummy2[i],dummy3[i]) <1) i--;
      }
      else i--;
    }
    else {

     ...
     char *argv[];
{
  ...
  double xpar[11],ypar[11],xmid,ymid,xm,xp,ym,yp;
  ...
  float xx,yy,bmag,scaleratio,srtolerance,ratio,x,y,dummy,X,Y;
  ...
  if ( argc != 9 ) {
     printf( "Usage: %s templist objlist matchoutfile mapfile closerthan totalmatchlimit skiprow wcsout\n", argv[0] );
     exit( 1 );
  }
  ...
  /*read in map file*/
  if( (success = fgets(line, 256, stream4)) == line) {
    i = sscanf(line, "%lf %lf %lf %lf", &xm, &xp, &ym, &yp);
    if(i == 4) {
      jtxform = 1;
    } else if(i==2) {
      jtxform = 0;
  ...
  for ( i = 1; i <= nstar1; i++ ) {
    ...
    for ( j = 1; j <= nstar2; j++ ) {
      ...
      if(jtxform) {
	x = ((x2[j]+0.5)-0.5*(xp+xm)) / (0.5*(xp-xm));
	y = ((y2[j]+0.5)-0.5*(yp+ym)) / (0.5*(yp-ym));
      } else {
	x = (x2[j]+0.5) / xmid - 2;
	y = (y2[j]+0.5) / ymid - 2;
      }

      X =   xpar[0] +   xpar[1]*x + xpar[2]*y +
        xpar[3]*x*x +  xpar[4]*x*y + xpar[5]*y*y +
        xpar[6]*x*x*x +  xpar[7]*x*x*y + xpar[8]*x*y*y + xpar[9]*y*y*y;
      Y =   ypar[0] +   ypar[1]*x + ypar[2]*y +
    ...
    if ( bestdist < limit ) {
      if (wcsout==0) {
	fprintf(stream3,"%8.2f %8.2f %8.2f %8.2f %8.2f %s %s %s\n",x2[bestj],y2[bestj],x1[i],y1[i],bestdist,dummy1[i],dummy2[i],dummy3[i]);
      }
      else {

...
{  
  ...
  char imagein[100],fringeim[100],imageout[100],sfim[100],indexx,list[100];
  char *header,*header1,*header2,line[150];
  ...
  if (sf ==1) {
    ...
    if (xaxis1 != xaxis2 || yaxis1 != yaxis2) {
      printf("superflat and fringe are not the same size, abort!\n"); 
      exit (-1);
    }
  }  
  
  
  if ((stream1 = fopen(list,"r"))== NULL) {
  ...
  while (feof(stream1) == 0) {
    ...
    */
    ...
    if(bitpix == -32) {
      ...
      wfitsreal(header, image, imagein);
...
char *s;
{
  printf("Syntax: defringe infilelist fringefile -superflat file -bitpix N  (16/-16/-32)\n");

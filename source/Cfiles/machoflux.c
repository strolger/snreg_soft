/* machoflux                              Thu Aug 19 12:23:13 EST 1999 sj  *
 *                                        modified BPS 27Jan00             *
...
#define BKG_ANNULUS_START 3.0
#define BKG_ANNULUS_WIDTH 8.0
...
main(int argc, char *argv[]) {
  ...
  int suma, sumb,sumzero;
  float maxa, flux, background, bkginap,mina,sumplus,summinus;
  ...
  bkg_inner_rad = aperture + BKG_ANNULUS_START;
  ...
  while (fgets(s,BUF,in)) {
	...
	if (r2 <= ap2) {    /* in the aperture */
	  if (sub-background > 0) sumplus+=sub-background;
	  else summinus+=sub-background;
	}
      }
    ...
    sumplus *= (M_PI*ap2/(float)suma);
    summinus *= (M_PI*ap2/(float)suma);

    bkginap = background * (M_PI*ap2);  /* how much background in aperture */
    
    printf("%8.2f %8.2f %10.0f %10.1f %9.1f %9.1f %10.0f %10.0f %4d\n",inx,iny,flux-bkginap,background,mina,maxa,sumplus,summinus,sumzero);
